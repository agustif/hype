...
Switching to Markdown mode shows the result immediately; saving writes that same document to disk. Source edits likewise update the thumbnail list. Empty slides must remain real source blocks, including at the beginning or end of the deck.

The right pane shows the selected slide as a preview. An Edit text action opens a compact per-slide Markdown editor below it. Keep layout automatic, with a few alignment and media controls. Direct canvas text editing is a later enhancement.

Markdown mode replaces the right pane with the whole source document. Selecting a thumbnail jumps to its source; moving the cursor to another slide updates selection. Switching modes retains the selected slide and edits. Visual operations modify the same source document.
...
Code fitting measures the widest line and total block height using actual font metrics, then chooses the largest size that fits both dimensions. Preserve indentation, expand tabs consistently, and never wrap or clip code silently. Warn when fitting would make it too small to read. Inline backtick spans inherit the surrounding text scale. Use KSyntaxHighlighting for language-tagged fences, initially validating Ruby, shell, YAML, JavaScript, and plain text. Avoid handwritten language lexers. Its [syntax-highlighting package is available in Arch Extra](https://archlinux.org/packages/extra/x86_64/syntax-highlighting/).
...
PowerPoint: a high-resolution rendering of each static slide placed edge-to-edge in a `.pptx`. Rendered slides preserve appearance and avoid font substitution. Editable PowerPoint text and elements are out of scope; all authoring happens in Hype or the Markdown source. Start at 1920 × 1080 per static slide, measure file size on a 150-slide deck, and use JPEG for photographic slides where acceptable while retaining PNG for text/code. Video slides include an embedded movie and poster, positioned over the static slide rendering. Render static content without duplicating the video poster underneath where transparency/cropping would cause artifacts.

Prototype a packaged Python helper with `python-pptx` for the PPTX container rather than immediately implementing Office XML in C++. Packaging is an explicit gate: [python-pptx is listed in the AUR](https://aur.archlinux.org/packages/python-pptx), so shipping may require maintaining an Omarchy package for it. Compare that maintenance cost with a narrowly scoped C++/ZIP writer after the video spike; do not assume handwritten movie relationships and timing XML are trivial. The desktop app remains C++/QML, with Python and python-pptx declared as export dependencies. [python-pptx documents movie insertion as experimental](https://python-pptx.readthedocs.io/en/latest/api/shapes.html#pptx.shapes.shapetree.SlideShapes.add_movie), so embedded video playback, fitting, and playback settings are an early compatibility gate. Test in actual PowerPoint as well as LibreOffice; a successful export file alone is insufficient.
...
3. **Build the visual editor.** Left thumbnails, right slide canvas, per-slide Markdown editing, Markdown toggle, drag reordering, keyboard navigation, and contextual media controls. Verify that dragging slide 5 between slides 1 and 2 moves its exact Markdown block to that position; adding and duplicating insert source blocks after selection; undo restores source and selection; and save/reopen preserves the result. Confirm mode switching never discards edits. Test empty slides at both document boundaries and duplication of media slides without extra asset copies.
